...
function loadSavedTeams() {
    ...
    fetch('/api/teams')
        ...
        .then(teams => {
            teams.forEach(team => {
                selectBox.append(`<option value="${team.id}">${team.name}</option>`);
            });
        })
...
$(document).ready(function () {
    ...
    $("#saveTeam").click(async function () {
        ...
        try {
            // Check if we're updating an existing team (one is selected)
            const selectedId = $("#savedTeams").val();
            if (selectedId) {
                await fetch(`/api/teams/${selectedId}`, {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ name: teamName, data: teamData })
                });
            } else {
                ...
                loadSavedTeams();
                setTimeout(() => {
                    $("#savedTeams").val(saved.id).trigger('change');
                }, 100);
                return;
            }
            loadSavedTeams();
            setTimeout(() => {
                $("#savedTeams").val(selectedId).trigger('change');
            }, 100);
        } catch (err) {
    ...
    // Delete a saved team
    $("#deleteTeam").click(async function () {
        const selectedId = $("#savedTeams").val();
